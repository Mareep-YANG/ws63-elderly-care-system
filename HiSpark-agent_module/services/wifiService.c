...
static td_void wifi_connection_changed(td_s32 state, const wifi_linked_info_stru *info, td_s32 reason_code)
{
    ...
    {
        log_error("%s::Connect fail!. try agin !\r\n", WIFI_STA_SAMPLE_LOG);
        g_wifi_state = WIFI_STA_SAMPLE_INIT;
    }
    else
    {
        log_info("%s::Connect succ!.\r\n", WIFI_STA_SAMPLE_LOG);
...
td_s32 example_get_match_network(wifi_sta_config_stru *expected_bss)
{
    ...
    td_char expected_ssid[] = "BACKUP";
    td_char key[] = "elab2025"; /* 待连接的网络接入密码 */
...
td_bool example_check_dhcp_status(struct netif *netif_p, td_u32 *wait_count)
{
    ...
    {
        /* DHCP成功 */
        log_info("%s::STA DHCP success.\r\n", WIFI_STA_SAMPLE_LOG);
        // DHCP成功之后启动WebSocket服务器
        return 0;
    }

    if (*wait_count > WIFI_GET_IP_MAX_COUNT)
    {
        log_warn("%s::STA DHCP timeout, try again !.\r\n", WIFI_STA_SAMPLE_LOG);
        *wait_count = 0;
        g_wifi_state = WIFI_STA_SAMPLE_INIT;
    }
    return -1;
...
td_s32 example_sta_function(td_void)
{
    ...
    do
    {
        ...
        else if (g_wifi_state == WIFI_STA_SAMPLE_SCAN_DONE)
        {
            ...
            {
                log_warn("%s::Do not find AP, try again !\r\n", WIFI_STA_SAMPLE_LOG);
                g_wifi_state = WIFI_STA_SAMPLE_INIT;
                continue;
            }
...
int sta_sample_init(void *param)
{
    ...
    {
        log_warn("%s::wifi_event_cb register fail.\r\n", WIFI_STA_SAMPLE_LOG);
        return -1;
    }
    log_info("%s::wifi_event_cb register succ.\r\n", WIFI_STA_SAMPLE_LOG);
    ...
    log_info("%s::wifi init succ.\r\n", WIFI_STA_SAMPLE_LOG);

    if (example_sta_function() != 0)
    {
        log_error("%s::example_sta_function fail.\r\n", WIFI_STA_SAMPLE_LOG);
        return -1;
    }
    else
    {
